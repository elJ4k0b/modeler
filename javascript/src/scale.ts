...
let scaledElements: Array<ContainerView> = [];
let scaleStartX = 0;
let scaleStartY = 0;
let scalePoint = 0;
let offset: Point = {x: 0, y: 0};

export function startscale(event: PointerEvent)
{
...
export function scale(event: PointerEvent)
{
...
export function endscale (event: PointerEvent)
{
    if(scaledElements == null) return;
    for(let element of scaledElements)
    {
