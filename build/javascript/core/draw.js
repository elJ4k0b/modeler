...
import { DroppedLineVisual, LineVisual, StraightLineVisual } from "../lines/line-visual.js";
let debug = true;
...
function draw_container(container, div = document.createElement("div")) {
    ...
    try {
        ...
        if (!titleContainerText)
            throw new Error(`TitleContainer does not have title text element on container with id ${container.id}`);
...
function draw_element(tableview, div = document.createElement("div")) {
    ...
    if (typeContainer) {
        ...
        let type = ((_a = typeMap.get(tableview.typeId)) === null || _a === void 0 ? void 0 : _a.label) || "none";
        ...
        try {
            let newTypeContainer = document.createElement("div");
            if (tableview.selected)
            ...
            title.style.textOverflow = "ellipsis";
            let type = ((_b = typeMap.get(tableview.typeId)) === null || _b === void 0 ? void 0 : _b.label) || "none";
    ...
    let title = titleContainer === null || titleContainer === void 0 ? void 0 : titleContainer.querySelector("span");
...
function draw_canvas() {
    ...
    */
    for (let container of diagview.containers.values()) {
        try {
            let containerDiv = document.getElementById(container.id);
            if (!containerDiv) {
    ...
    for (let tableview of diagview.tableviews.values()) {
        try {
            let elementDiv = document.getElementById(tableview.id);
            if (!elementDiv) {
...
function draw_overlay() {
    ...
    for (let view of overlayableElements) {
        ...
        if (diagview.is_selected(view.id)) {
            ...
            try {
                let points = (_a = overlay.getAttribute("points")) === null || _a === void 0 ? void 0 : _a.split(" ");
                if (!points)
                    throw new Error("Points attribute of overlay was not set");
    ...
    for (let group of overlayContainerSvg.children) {
        try {
            let id = (_b = group.getAttribute("overlay-id")) === null || _b === void 0 ? void 0 : _b.toString();
            if (!id)
                throw new Error(`Group ${group} lacks overlay-id attribute`);
...
function draw_pointers(event) {
    let pointerLayer = document.getElementById("pointer-layer");
    if (!pointerLayer) {
        log(`Failed to get pointer Layer`, "error", { file: "draw", line: 584, "method": "draw_pointers" });
        return;
    }
    //pointerLayer.innerHTML = "";
    const randomX = Math.random() * 1000;
    const randomY = Math.random() * 1000;
    let pointer = pointerLayer.querySelector("#pointer");
    if (!pointer) {
        pointer = document.createElementNS("http://www.w3.org/2000/svg", "rect");
        pointer.setAttribute("fill", "black");
        pointer.id = "pointer";
        pointer.setAttribute("width", "150");
        pointer.setAttribute("height", "50");
        pointer.setAttribute("rx", "1.5rem");
        pointer.style.transition = "transform 0.25s ease";
        pointerLayer.appendChild(pointer);
    }
    pointer.addEventListener("transitionend", () => marker === null || marker === void 0 ? void 0 : marker.remove());
    pointer.setAttribute("transform", `translate(${event.clientX} ${event.clientY})`);
    let marker = pointerLayer.querySelector("#marker");
    if (!marker) {
        marker = document.createElementNS("http://www.w3.org/2000/svg", "circle");
        marker.setAttribute("fill", "red");
        marker.id = "marker";
        marker.setAttribute("r", "30");
        marker.setAttribute("z-index", "10000");
        pointerLayer.appendChild(marker);
    }
    const position = pointer.getCTM();
    if (position && position.e && (position === null || position === void 0 ? void 0 : position.f)) {
        const originPosition = { x: position.e + 75, y: position.f + 25 };
        const vector = { x: event.clientX - (originPosition.x), y: event.clientY - originPosition.y };
        ...
        const targetPos = { x: originPosition.x + 100 * normalizedVector.x, y: originPosition.y + 40 * normalizedVector.y };
        marker.setAttribute("transform", `translate(${targetPos.x} ${targetPos.y})`);
    }
}
