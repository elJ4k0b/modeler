...
const MAX_FOCUS_DISTANCE = 0.9;
const AUTOSCROLL = false; //SWITCH TO TOGGLE AUTOSCROLLING TO ELEMENTS IN FOCUS
...
export function select(event: PointerEvent)
{
    ...
    if(!already_selected && !element_is_in_focus(element) && AUTOSCROLL)
         scroll_to_selection();
    draw();
}
...
function element_is_in_focus(element: View)
{
    let elementMidX = element.position.left + element.dimension.width/2;
    let elementMidY = element.position.top + element.dimension.height/2;
    let viewportCenter = zoomHandler.viewportCenter;
    let windowDimension = zoomHandler.getWindowDimension();
    console.log(windowDimension);
    console.log(elementMidX, elementMidY);
    let zoomFactor = zoomHandler.zoomFactor;

    //wenn Element in der Mitte: windowCenter  - (elementCenter + viewportCenter) = 0
    // -> Wenn Element nicht inder Mitte: windowCenter  - (elementCenter + viewportCenter) = distance
    //-> distanceX = windowCenter.x - (elementMidX * zoomFactor + viewportCenter.x)
    let distanceX = Math.abs(windowDimension.width/2 - (elementMidX * zoomFactor + viewportCenter.x));
    let distanceY = Math.abs(windowDimension.height/2 - (elementMidY * zoomFactor + viewportCenter.y));
    console.log(`Distance to viewport center: ${distanceX}, ${distanceY}`);
    console.log(`Max distance to be in focus: ${windowDimension.width/2*0.5}, ${windowDimension.height/2*MAX_FOCUS_DISTANCE}`);
    return distanceX < windowDimension.width/2*0.5 && distanceY < windowDimension.height/2*MAX_FOCUS_DISTANCE;
}
