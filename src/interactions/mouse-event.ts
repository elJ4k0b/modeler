...
const DOUBLE_CLICK_TIMEOUT_TIME = 350;
...
export type CustomPointer = {
    ...
    longclickTimeout: ReturnType<typeof setTimeout>,
    drag: boolean,
...
class MouseEvent
{
    ...
    protected timeOfLastPointerCreation: number|undefined;
    ...
    private _handleDoubleClick(event: PointerEvent, pointer: CustomPointer)
    {
        this.handleDoubleClick(event, pointer);
    }
    ...
    private _createPointer(event: PointerEvent): CustomPointer
    {
        ...
        if(this.timeOfLastPointerCreation && Date.now() - this.timeOfLastPointerCreation  < DOUBLE_CLICK_TIMEOUT_TIME)
            this._handleDoubleClick(event, pointer);
        
        this.timeOfLastPointerCreation = Date.now();
    ...
    protected handleDoubleClick(event: PointerEvent, pointer: CustomPointer){}
