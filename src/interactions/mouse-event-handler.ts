import MouseEvent, { CustomPointer } from "./mouse-event.js";
...
import { startpinch, pinch} from "./pinch-zoom.js";
...
class CustomEvents extends MouseEvent
{
    ...
    protected override handleDoubleClick(event: PointerEvent, pointer: CustomPointer): void {
        const clickedTargets = document.elementsFromPoint(event.clientX, event.clientY).filter(target => diagview.get_element(target.id) || diagview.get_container(target.id) || diagview.get_lineview(target.id));
        const firstHit = clickedTargets[0];
        if(firstHit)
            notify("double-click", {pointer: pointer, target:firstHit});
    }
