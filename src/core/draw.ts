...
import { DashedLineVisual, DroppedLineVisual, LineVisual, StraightLineVisual } from "../lines/line-visual.js";
...
function draw_pointers(event: PointerEvent)
{
    ...
    let pointer = pointerLayer.querySelector("#pointer") as SVGGraphicsElement;
    ...
    pointer.addEventListener("transitionend", () => marker?.remove());
    ...
    if(!marker)
    {
        marker = document.createElementNS("http://www.w3.org/2000/svg", "circle") as SVGCircleElement;
    ...
    if(position && position.e && position?.f)
    {
